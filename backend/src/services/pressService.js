...
const completePressProcess = (
  ...
  return new Promise((resolve, reject) => {
    const query = `UPDATE press_processes SET status = 'COMPLETED', return_weight = ?, return_pieces = ?, scrap_weight = ?, loss_weight = ?, end_time = CURRENT_TIMESTAMP, description = COALESCE(?, description) WHERE id = ?`;
    db.run(
      query,
