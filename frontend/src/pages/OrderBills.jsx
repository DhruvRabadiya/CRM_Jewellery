...
// Purity string for a given metal type
const metalPurityFor = (metalType) =>
  metalType === "Gold 22K" ? "91.67" : metalType === "Silver" ? "99.90" : "99.99";

// Normalise any legacy payment_entries into three fixed slots used by the form:
//   [0] Metal  (weight + reference_rate)
//   [1] Cash   (amount)
//   [2] Bank / UPI (amount)
// settlementRate: the saved rate/10g for this metal (used as fallback when payment entries
// have no reference_rate).
const toFixedPayments = (entries = [], metalType = "Gold 24K", settlementRate = 0) => {
  const metal    = entries.find((e) => e.payment_type === "Metal") || {};
  const cashSum  = entries
    .filter((e) => e.payment_type === "Cash")
    .reduce((s, e) => s + (parseFloat(e.amount) || 0), 0);
  const bankSum  = entries
    .filter((e) => e.payment_type === "Bank / UPI" || e.payment_type === "Bank")
    .reduce((s, e) => s + (parseFloat(e.amount) || 0), 0);
  const refRate  = metal.reference_rate || (settlementRate > 0 ? String(settlementRate) : "");
  return [
    {
      payment_type:   "Metal",
      amount:         "",
      ...
      weight:         metal.weight     || "",
      purity:         metal.purity     || metalPurityFor(metalType),
      reference_rate: refRate,
    },
    { payment_type: "Cash",       amount: cashSum > 0 ? String(cashSum) : "", metal_type: "Gold 24K", weight: "", purity: "", reference_rate: "" },
    { payment_type: "Bank / UPI", amount: bankSum > 0 ? String(bankSum) : "", metal_type: "Gold 24K", weight: "", purity: "", reference_rate: "" },
  ];
};
  ...
  React.useEffect(() => {
    ...
    Promise.all([
      ...
      .then(([list, pref]) => {
        ...
        if (pref?.printerName && !isPdfPrinter(pref.printerName)) {
          setSelectedPrinter(pref.printerName);
        } else {
          const real = printerList.find((p) => !isPdfPrinter(p.name));
          const def  = printerList.find((p) => p.isDefault);
          const pick = real || def;
          if (pick) setSelectedPrinter(pick.name);
        }
      })
      .catch(() => {})
      ...
      >
        ...
        <div style={{ padding: "0 16px 10px", display: "flex", alignItems: "center", gap: 10 }}>
          ...
          {printersLoading ? (
            ...
            <span style={{ fontSize: 12, color: "#b45309", display: "flex", alignItems: "center", gap: 8 }}>
              No printers detected automatically
              <button
                onClick={() => { window.electronAPI?.getPrinters().then((list) => { if (list?.length) { setPrinters(list); const real = list.find((p) => !isPdfPrinter(p.name)); const def = list.find((p) => p.isDefault); const pick = real || def; if (pick) setSelectedPrinter(pick.name); } }); }}
                style={{ fontSize: 11, padding: "2px 8px", borderRadius: 4, border: "1px solid #b45309", background: "#fef3c7", color: "#92400e", cursor: "pointer" }}
              >Retry</button>
              <span style={{ color: "#94a3b8" }}>or click Print to use system dialog</span>
            </span>
          ) : (
            <select
            ...
            >
              {[...printers].sort((a, b) => isPdfPrinter(a.name) - isPdfPrinter(b.name)).map((p) => (
                <option key={p.name} value={p.name}>
                  {p.name}{p.isDefault ? "  Default" : ""}{isPdfPrinter(p.name) ? "  (PDF)" : ""}
                </option>
              ))}
            </select>
          )}
      ...
      <div style={{ display: "flex", justifyContent: "center", padding: "32px 16px 48px" }}>
        ...
        >
          ...
          {products.map((metalType) => {
            ...
            return (
              <div key={metalType} style={{ marginBottom: 16 }}>
                ...
                {(() => {
                  const totalPcs    = metalItems.reduce((s, it) => s + (parseInt(it.pcs, 10) || 0), 0);
                  const totalWeight = metalItems.reduce((s, it) => s + ((parseFloat(it.size_value) || 0) * (parseInt(it.pcs, 10) || 0)), 0);
                  const totalLabour = metalItems.reduce((s, it) => s + ((parseFloat(it.lc_pp) || 0) * (parseInt(it.pcs, 10) || 0)), 0);
                  return (
          ...
          <div style={{ marginLeft: "auto", width: 260, fontSize: 12, border: "1px solid #cbd5e1", padding: "12px 14px", marginTop: 20 }}>
            ...
            {Object.entries(summary.settlementRate || {}).some(([, r]) => r > 0) && (
              <div style={{ marginTop: 10, paddingTop: 8, borderTop: "1px dashed #cbd5e1" }}>
                {Object.entries(summary.settlementRate || {}).map(([mt, rate]) => {
                  if (!rate || rate <= 0) return null;
                  return (
                    <div key={mt} style={{ display: "flex", justifyContent: "space-between", padding: "2px 0", fontSize: 11 }}>
                      <span style={{ color: "#64748b" }}>{mt} Rate / 10g</span>
                      <span style={{ fontWeight: 700, color: "#92400e" }}>&#8377;{Number(rate).toLocaleString("en-IN")}</span>
                    </div>
                  );
                })}
              </div>
...
// --- State ---
  ...
  // Global unified picker for the flat items table
  const [globalPicker, setGlobalPicker] = useState({ show: false, category: "", size_label: "" });
  ...
  const [billType, setBillType]           = useState("estimate"); // "estimate" | "order"
  ...
  const firstPaymentRef = useRef(null);
...
// --- State ---
  ...
  const resetForm = useCallback(async (charges = {}, nextDate = getTodayLocalISO()) => {
    ...
    setPaymentEntries(toFixedPayments([], "Gold 24K"));
...
// --- State ---
  // Single-select: clicking a metal makes it the only selected metal.
  // Clears items/addedKeys from the previous metal and syncs the Metal payment slot.
  const handleProductSelect = useCallback((metalType) => {
    setSelectedProducts((cur) => {
      if (cur.length === 1 && cur[0] === metalType) return cur; // no-op
      setItems((prev) => buildItemsFromCharges(groupedCharges, [metalType], customerType, prev));
    ...
    setPaymentEntries((cur) => {
      ...
      return cur.map((e, i) =>
        i === 0 ? { ...e, metal_type: metalType, purity: metalPurityFor(metalType) } : e
      );
    });
  ...
  const confirmGlobalAdd = useCallback((overrideSize, overrideCategory) => {
    ...
    setItems((prev) => prev.map((item) => {
      if (item.metal_type === metalType && item.category === category && item.size_label === size_label && (parseInt(item.pcs, 10) || 0) === 0) {
        return { ...item, pcs: "" };
      }
      return item;
    }));
    setGlobalPicker((p) => ({ ...p, show: false, size_label: "" }));
    setTimeout(() => { if (pcsInputRefs.current[key]) pcsInputRefs.current[key].focus(); }, 60);
  }, [globalPicker, selectedProducts]);
  ...
  const openEdit = useCallback(async (bill) => {
    try {
      ...
      const savedRates = extractSettlementRates(full);
      setSettlementRates(savedRates);
      setPaymentEntries(toFixedPayments(
        normalizePaymentEntries(full.payment_entries || [], full),
        singleMetal,
        savedRates[singleMetal] || 0,
      ));
      ...
      const builtItems = buildItemsFromCharges(charges, [singleMetal], full.customer_type || "Retail", full.items || []);
      setItems(builtItems);
      setAddedKeys(new Set(
        builtItems
  ...
  const quickTokenPrint = useCallback(async (bill) => {
    const isElectron = typeof window !== "undefined" && typeof window.electronAPI !== "undefined";
    if (!isElectron) { alert("Token printing is only available in the desktop app."); return; }
    try {
      const full = await getOrderBill(bill.id);
      const products = parseProducts(full.products);
      const paymentEntries = normalizePaymentEntries(full.payment_entries || [], full);
      const summary = computeEstimateBalance(
        full.items || [], paymentEntries, full.discount, extractSettlementRates(full)
      );
      const isRetail = (full.customer_type || "Retail").toLowerCase() === "retail";
      const estimateData = {
        billNo: full.ob_no, date: full.date,
        customerName: full.customer_name || "", customerPhone: full.customer_phone || "",
        customerType: full.customer_type || "Retail", isRetail, products,
        items: (full.items || []).map((item) => {
          const pcs    = parseInt(item.pcs, 10)    || 0;
          const weight = parseFloat(item.weight)   || ((parseFloat(item.size_value) || 0) * pcs);
          const tLc    = parseFloat(item.t_lc)     || ((parseFloat(item.lc_pp)     || 0) * pcs);
          return { metal_type: item.metal_type || "Gold 24K", category: item.category || "Standard",
                   size_label: item.size_label || "", pcs, weight,
                   lc_pp: parseFloat(item.lc_pp) || 0, t_lc: tLc };
        }),
        summary: {
          totalPcs: summary.totalPcs || 0, totalWeight: summary.totalWeight || 0,
          labourTotal: summary.labourTotal || 0, labourAfterDiscount: summary.labourAfterDiscount || 0,
          discount: summary.discount || 0, moneyPaid: summary.moneyPaid || 0,
          totalAmount: summary.totalAmount || 0, cashDue: summary.cashDue || 0,
          amountDue: summary.amountDue || 0, amountGiven: summary.amountGiven || 0,
          refundDue: summary.refundDue || 0,
          requiredMetal: summary.requiredMetal || {}, metalReceived: summary.metalReceived || {},
          metalDue: summary.metalDue || {}, metalDueUnsettled: summary.metalDueUnsettled || {},
          metalShortfallSettled: summary.metalShortfallSettled || {},
          metalCredit: summary.metalCredit || {}, settlementRate: summary.settlementRate || {},
        },
      };
      let printerName = (await window.electronAPI.getPrinterPref().catch(() => ({}))).printerName || "";
      if (!printerName || isPdfPrinter(printerName)) {
        const list = await window.electronAPI.getPrinters().catch(() => []);
        const real = list.find((p) => !isPdfPrinter(p.name));
        if (!real) { alert("No real printer found. Please connect the TSC printer and try again."); return; }
        await window.electronAPI.savePrinterPref({ printerName: real.name }).catch(() => {});
        printerName = real.name;
      }
      const result = await window.electronAPI.printEstimate({ ...estimateData, printerName });
      if (!result?.ok) showToast("Token print failed: " + (result?.error || "Unknown error"), "error");
      else showToast("Token sent to printer");
    } catch (err) {
      showToast("Token print error: " + err.message, "error");
    }
  }, [showToast]);
                          ...
                          >
                            ...
                            <td className="px-3 py-3">
                              <div className="flex items-center justify-center gap-1">
                                ...
                                <button
                                  onClick={() => quickTokenPrint(bill)}
                                  title="Print Token (thermal)"
...
// --- State ---
  ...
  const allPcsKeys = allActiveItems
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm">
        ...
        {/* Row 1: Back . Ref# . Date . Status . Save buttons */}
        <div className="flex items-center gap-2 px-4 py-2.5 bg-slate-50 border-b border-slate-100 flex-wrap rounded-t-2xl">
          <button
            type="button"
            onClick={() => setView("list")}
            className="flex items-center gap-1.5 text-xs font-bold text-slate-500 hover:text-indigo-600 bg-white border border-slate-200 px-2.5 py-1.5 rounded-lg hover:border-indigo-300 transition-colors flex-shrink-0"
          >
            <ArrowLeft size={13} /> Estimates
          </button>

          <div className="flex items-center gap-1 border border-slate-200 rounded-lg bg-white px-2.5 py-1.5 flex-shrink-0">
            <span className="text-[10px] font-black text-slate-400">#</span>
            <input
              type="number" min="1" value={obNo} onChange={(e) => setObNo(e.target.value)}
              className="w-12 text-xs font-bold text-slate-600 focus:outline-none bg-transparent"
            />
          </div>

          <input
            type="date" value={formDate} onChange={(e) => setFormDate(e.target.value)}
            className="px-2.5 py-1.5 text-xs border border-slate-200 rounded-lg focus:outline-none focus:ring-1 focus:ring-indigo-300 bg-white font-semibold text-slate-700 flex-shrink-0"
            style={{ colorScheme: "light" }}
          />
          ...
          <div className="flex gap-1 flex-shrink-0">
            {BILL_STATUS_LIST.map((status) => {
              ...
              return (
                <button
                  key={status} type="button" onClick={() => setBillStatus(status)}
          ...
          {billStatus === "Pending" && (
            <div className="flex items-center gap-1.5 flex-shrink-0">
              <span className="text-[10px] font-bold text-amber-600 whitespace-nowrap">Deliver by:</span>
              <input
                type="date" value={deliveryDate} onChange={(e) => setDeliveryDate(e.target.value)}
                className="px-2 py-1.5 text-xs border border-amber-200 rounded-lg focus:outline-none focus:ring-1 focus:ring-amber-300 bg-amber-50/50 font-semibold"
                style={{ colorScheme: "light" }}
              />
            </div>
          )}
          ...
          <button
            type="button" onClick={() => handleSave(false)}
            disabled={saving || validatingStock || (isStockActive(billStatus) && !stockValidation.valid)}
            className={"flex items-center gap-1.5 px-4 py-1.5 text-xs font-black rounded-lg text-white transition-colors flex-shrink-0 disabled:opacity-50 " + getStatusConfig(billStatus).buttonClass}
          >
            <Save size={13} /> {saving ? "Saving..." : editBill ? "Update" : "Save"}
          </button>
          <button
            type="button" onClick={() => handleSave(true)}
            disabled={saving || validatingStock || (isStockActive(billStatus) && !stockValidation.valid)}
            className="flex items-center gap-1.5 px-4 py-1.5 text-xs font-black rounded-lg text-white bg-slate-700 hover:bg-slate-800 disabled:opacity-50 transition-colors flex-shrink-0"
          >
            <Printer size={13} /> Save & Print
          </button>
        </div>

        {/* Row 2: Metal types . Customer . Type . Phone/Addr toggle */}
        <div className="flex items-center gap-2 px-4 py-2.5 flex-wrap">
          <span className="text-[10px] font-black text-slate-400 uppercase tracking-wider flex-shrink-0">Metal:</span>
          <div className="flex gap-1 flex-shrink-0">
            {METAL_TYPES.map((mt) => {
              const selected = selectedProducts[0] === mt;
              return (
                <button
                  key={mt} type="button" onClick={() => handleProductSelect(mt)}
          ...
          <div className="flex-1 min-w-[200px] max-w-sm">
            <CustomerNameCombo
              value={customerName} linkedCustomer={selectedCustomer}
              onChange={(name) => setCustomerName(name)}
              onSelect={(c) => {
                setSelectedCustomer(c); setCustomerName(c.party_name || "");
                setCustomerPhone(c.phone_no || ""); setCustomerAddress(c.address || "");
          ...
          {!showCustomerDetails ? (
            <button type="button" onClick={() => setShowCustomerDetails(true)}
              className="text-[10px] font-bold text-indigo-500 hover:text-indigo-700 flex items-center gap-0.5 flex-shrink-0">
              <Plus size={10} /> Phone/Addr
            </button>
          ) : (
            <button type="button"
              onClick={() => { setShowCustomerDetails(false); setCustomerPhone(""); setCustomerAddress(""); }}
              className="text-[10px] font-bold text-rose-400 hover:text-rose-600 flex items-center gap-0.5 flex-shrink-0">
              <X size={10} /> Remove
            </button>
          )}
        </div>
        ...
        {showCustomerDetails && (
          <div className="flex gap-3 px-4 pb-3 pt-2.5 flex-wrap border-t border-slate-100">
            <div className="flex-1 min-w-[140px]">
              <label className="block text-[10px] font-black text-slate-400 uppercase tracking-wider mb-1">Phone</label>
              <input type="text" value={customerPhone}
                onChange={(e) => { setSelectedCustomer(null); setCustomerPhone(e.target.value); }}
                placeholder="Phone number"
                className="w-full px-2.5 py-1.5 text-xs border border-slate-200 rounded-lg focus:outline-none focus:ring-1 focus:ring-indigo-300 bg-slate-50" />
            </div>
        ...
        {summary.totalPcs > 0 && (() => {
          ...
          return (
            <div className="mx-4 mb-3 flex items-start gap-2 px-3 py-2 bg-amber-50 border border-amber-200 rounded-xl">
              <AlertCircle size={13} className="text-amber-500 flex-shrink-0 mt-0.5" />
              <p className="text-xs text-amber-700 font-semibold">
                Balance is due - add the customer's name above so it can be tracked in their ledger.
              </p>
            </div>
          );
        })()}
          ...
          <table className="w-full text-sm min-w-[600px]">
            <thead>
              <tr className="bg-slate-50 border-b border-slate-200 text-[10px] uppercase tracking-wider text-slate-400">
                ...
                <th className="px-3 py-2.5 text-right font-black">LC/pc (Rs.)</th>
                <th className="px-3 py-2.5 text-center font-black">QTY</th>
                <th className="px-3 py-2.5 text-right font-black">Tot. Wt</th>
                <th className="px-3 py-2.5 text-right font-black">Labour</th>
                <th className="w-8" />
              </tr>
            </thead>
            <tbody>
                  ...
                  return (
                    <tr key={key}
                      className={`border-b border-slate-100 last:border-b-0 transition-colors ${
                        pcs > 0 ? "bg-indigo-50/25" : "hover:bg-slate-50/40"
                      }`}
                    >
                      ...
                      <td className="px-3 py-2">
                        ...
                        {validation && (
                          <p className={`text-[9px] font-bold leading-tight ${
                            !validation.valid ? "text-rose-600" :
                            (validation.available_pieces || 0) === 0 ? "text-slate-400" :
                            "text-emerald-600"
                          }`}>
                            {!validation.valid
                      ...
                      <td className="px-3 py-2 text-right text-xs font-mono text-slate-500">{fmt(item.size_value, 4)}</td>
                      <td className="px-3 py-2 text-right text-xs font-mono text-slate-500">{fmt(item.lc_pp, 0)}</td>
                      <td className="px-3 py-2 text-center">
                        <input
                          ...
                          className={`w-16 text-center text-sm border rounded-lg px-1.5 py-1.5 focus:outline-none focus:ring-2 transition-colors font-bold ${
                            validation && !validation.valid
                              ? "border-rose-300 bg-rose-50 text-rose-700 focus:ring-rose-300"
                              : "border-indigo-200 bg-white text-indigo-700 focus:ring-indigo-300"
                          }`}
                          placeholder="0"
                        />
                      </td>
                      <td className="px-3 py-2 text-right text-xs font-mono text-slate-700">{pcs > 0 ? fmt(weight, 4) : "-"}</td>
                      <td className="px-3 py-2 text-right text-xs font-bold text-slate-800">{pcs > 0 ? fmt(labour, 0) : "-"}</td>
                      <td className="px-3 py-2 text-center">
                        <button type="button"
                          onClick={() => {
              ...
              {globalPicker.show ? (
                <div className="flex items-end gap-2 flex-wrap">
                    ...
                    >
                      ...
                      {(metalCats[effectiveCategory] || [])
                        ...
                        .map((row) => {
                          ...
                          const suffix    = avail === null ? "" : avail === 0 ? "  - not in stock" : `  - ${avail} in stock`;
                          return <option key={row.size_label} value={row.size_label}>{row.size_label}{suffix}</option>;
                        })}
                    </select>
                  </div>
                  <div className="flex gap-2 pb-0.5">
                    ...
                    <button type="button"
                      onClick={() => setGlobalPicker((p) => ({ ...p, show: false, size_label: "" }))}
                      className="px-3 py-1.5 text-sm font-semibold text-slate-500 bg-white border border-slate-200 rounded-lg hover:bg-slate-50">
                      Cancel
                    </button>
                  </div>
                </div>
              ) : Object.keys(metalCats).length > 0 ? (
                <button type="button"
                  onClick={() => {
                    const firstCat = Object.keys(metalCats)[0] || "";
                    setGlobalPicker({ show: true, category: firstCat, size_label: "" });
                  }}
                  className="flex items-center gap-2 text-sm font-bold text-indigo-600 hover:text-indigo-800 hover:bg-indigo-50 border border-dashed border-indigo-200 rounded-xl px-4 py-2 transition-all"
                >
                  <Plus size={14} /> Add Row
                </button>
              ) : (
                <p className="text-xs text-slate-400">No sizes configured. Add them in Admin - Labour Charges.</p>
              )}
            </div>
          );
        })()}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        {/* == Payment Received == fixed 4 rows == */}
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
          <div className="px-4 py-3 border-b border-emerald-100 bg-gradient-to-r from-emerald-50 to-white">
            <h2 className="font-black text-slate-700 text-sm">Payment Received</h2>
            <p className="text-[10px] text-slate-400 font-medium mt-0.5">
              {selectedProducts[0] || "Gold 24K"} | purity {paymentEntries[0]?.purity || metalPurityFor(selectedProducts[0] || "Gold 24K")}
            </p>
          </div>
          <div className="px-4 py-4 space-y-3">
            ...
            {/* Row 1 - Metal Weight */}
            <div>
              <label className="block text-[10px] font-black text-amber-700 uppercase tracking-wider mb-1.5">
                Metal Weight (g)
              </label>
              <input
                ref={firstPaymentRef}
                type="number" min="0" step="0.001"
                value={paymentEntries[0]?.weight || ""}
                onChange={(e) => updatePaymentEntry(0, "weight", e.target.value)}
                className="w-full px-3 py-2 text-sm border border-amber-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-amber-300 bg-amber-50/40 font-semibold placeholder:text-amber-300"
                placeholder="0.000"
              />
            </div>
            ...
            <div>
              ...
              <div className="relative">
                <span className="absolute left-3 top-1/2 -translate-y-1/2 text-xs font-bold text-amber-400 pointer-events-none">Rs.</span>
                <input
                  type="number" min="0" step="1"
                  value={paymentEntries[0]?.reference_rate || ""}
                  onChange={(e) => {
                    const v = e.target.value;
                    updatePaymentEntry(0, "reference_rate", v);
                    updateSettlementRate(selectedProducts[0] || "Gold 24K", v);
                  }}
                  className="w-full pl-7 pr-3 py-2 text-sm border border-amber-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-amber-300 bg-amber-50/40 font-semibold placeholder:text-amber-300"
                  placeholder="0"
                />
              </div>
              {(parseFloat(paymentEntries[0]?.weight) > 0 && parseFloat(paymentEntries[0]?.reference_rate) > 0) && (
                <p className="text-[10px] text-amber-600 font-semibold mt-1 text-right">
                  Metal value - {fmtMoney(roundMoney((parseFloat(paymentEntries[0].weight) * parseFloat(paymentEntries[0].reference_rate)) / 10))}
                </p>
              )}
            </div>
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  value={paymentEntries[1]?.amount || ""}
                  onChange={(e) => updatePaymentEntry(1, "amount", e.target.value)}
                  className="w-full pl-7 pr-3 py-2 text-sm border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-300 bg-white font-semibold"
                  placeholder="0.00"
                />
              </div>
            </div>
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  type="number" min="0" step="0.01"
                  value={paymentEntries[2]?.amount || ""}
                  onChange={(e) => updatePaymentEntry(2, "amount", e.target.value)}
                  className="w-full pl-7 pr-3 py-2 text-sm border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-300 bg-white font-semibold"
                  placeholder="0.00"
                />
              </div>
            ...
            <div className="px-5 py-4 space-y-2.5">
              ...
              {summary.totalPcs > 0 && (() => {
                ...
                const cfg = isFullyPaid
                  ? { bg: "bg-emerald-50", border: "border-emerald-200", label: "Paid",   labelCls: "text-emerald-700",  sub: "All settled" }
                  : hasRefund
              ...
              {/* Rate-not-set warning - shown when items have weight but no rate is entered yet */}
              {summary.totalWeight > 0 && !(parseFloat(settlementRates?.[selectedProducts[0]]) > 0) && (
                <div className="rounded-xl px-3 py-2 border bg-amber-50 border-amber-200 text-xs text-amber-700 font-semibold">
                  ! Enter Rate / 10g in Payment Received - metal value not included yet
                </div>
              )}
